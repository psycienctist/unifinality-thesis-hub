              ...
              <div id="map" className="section diagram-section">
                <div className="diagram-grid">
                  <div className="loop-map-panel">
                    <div className="loop-map" data-active-step={activeMapNode} role="img" aria-label="Interactive Recognition Loop architecture map">
                      <svg viewBox="0 0 520 300" aria-hidden="true">
                        <path className="loop-path loop-path-primary" d="M260 43 C420 43 475 111 449 188 C425 259 350 276 260 267 C170 276 95 259 71 188 C45 111 100 43 260 43" />
                        <path className="loop-path loop-path-secondary" d="M260 43 C100 43 45 111 71 188 C95 259 170 276 260 267" />
                        {mapNodes.map((node, index) => { const positions = [[260, 38], [410, 100], [407, 218], [260, 267], [112, 218], [110, 100]][index]; return <g key={node.id} className={`map-node ${activeMapNode === node.id ? "active" : ""}`} role="button" tabIndex={0} onClick={() => setActiveMapNode(node.id)} onMouseEnter={() => setActiveMapNode(node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setActiveMapNode(node.id); }} aria-label={`Show ${node.label} step`} aria-pressed={activeMapNode === node.id}><circle cx={positions[0]} cy={positions[1]} r="27" /><text x={positions[0]} y={positions[1] - 3} textAnchor="middle">{String(index + 1).padStart(2, "0")}</text><text className="map-node-label" x={positions[0]} y={positions[1] + 45} textAnchor="middle">{node.label}</text></g>; })}
                    ...
                    <div className="map-detail"><p className="eyebrow">02 / Recognition Loop architecture</p><h3>Relation becomes a protocol.</h3><p>The loop turns the thesis’s ethical intuition into an inspectable sequence. Choose a node to see the operative question at that point in the cycle.</p><div key={activeMapNode} className="map-detail-note"><span className="meta">ACTIVE STEP / {mapNodes.find((node) => node.id === activeMapNode)?.label.toUpperCase()}</span><p>{mapNodes.find((node) => node.id === activeMapNode)?.detail}</p></div><a href="./library/Recognition_Loop_Multi_Agent_Architecture_Specification.md" target="_blank" rel="noreferrer">Read the full architecture specification <ArrowDownRight size={15} /></a></div>
                  </div>
